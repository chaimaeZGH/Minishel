...
/*   Updated: 2025/07/26 22:51:54 by czghoumi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_expend	*expand_content(const char *content, char **env)
{
	t_expend	*result;
	size_t	bufsize;
	char	quote_char;
	...
	while (content[i])
	{
		...
		if (c == '$' && quote_char != 1) 
		{
			...
			len_result = ft_strlen(result->str);
		...
		else
		{
			...
			{
				bufsize = len + 2;
				result->str = realloc(result->str, bufsize);
				if (!(result->str))
					return (NULL);
			}
			result->str[len] = c;
...
void	process_expend_content(t_tokenlist *token, char **env)
{
	...
	if (!token || !token->content)
		return ;
	ss = expand_content(token->content, env);
	new_content = ss->str;
	if (ss->i == 0)
	{
		free(token->content);
